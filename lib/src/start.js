 ...
 **/
const getProxy = ({ proxyPrefix, proxyHost }) => proxyPrefix.length === proxyHost.length && proxyPrefix.length > 0
// (typeof proxyPrefix === 'string' && typeof proxyHost === 'string')
...
function getRequestType ({ pathname, fileType }, { proxyPrefix, pushstate }) {
  if (proxyPrefix.map(pp => new RegExp(`^${pp}`)).some(regex => regex.test(pathname))) {
    return PROXY_TYPE
  } else if (
...
function handler (model) {
  ...
  return function (req, res) {
    ...
    if (proxy && url.isType(PROXY_TYPE)) {
      const index = model.proxyPrefix.findIndex(pp => new RegExp(`^${pp}`).test(req.url))
      req.url = req.url.replace(model.proxyPrefix[index], '')
      proxy.web(req, res, {
        target: model.proxyHost[index],
        changeOrigin: true,
        hostRewrite: true,
